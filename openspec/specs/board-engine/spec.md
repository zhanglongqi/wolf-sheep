...
Defines the board topology (nodes, adjacency) and the `Board` engine class that tracks piece occupancy and answers positional queries.

## Requirements

### Requirement: Board config defines node positions and adjacency
The system SHALL define one board configuration (`grid5x5`) as a plain data object. The config SHALL include a `nodes` map of node IDs to `{x, y}` pixel coordinates, an `adjacency` map of node IDs to arrays of neighbor IDs (bidirectional), `wolfStart` (initial wolf node IDs), `sheepStart` (initial on-board sheep node IDs), and `sheepReserve` (count of sheep entering via placement — currently 0, since the only board has no placement phase).
